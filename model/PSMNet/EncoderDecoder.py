...
class StackHourglass(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.conv_in1 = nn.Sequential(nn.Conv3d(in_channels=64, out_channels=32, kernel_size=3, padding=1, stride=1),nn.BatchNorm3d(32),nn.ReLU(inplace=True),
                                      nn.Conv3d(in_channels=32, out_channels=32, kernel_size=3, padding=1, stride=1),nn.BatchNorm3d(32),nn.ReLU(inplace=True))
        self.conv_in2 = nn.Sequential(nn.Conv3d(in_channels=32, out_channels=32, kernel_size=3, padding=1, stride=1),nn.BatchNorm3d(32),nn.ReLU(inplace=True),
